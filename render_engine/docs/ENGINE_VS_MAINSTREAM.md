# 与主流引擎能力对标（自评）

> **性质：** 工程观感自评，不是跑分或功能勾选表。  
> **基准日期：** 2026-08-20（W20 封板，HEAD 见 [DOING_UNDO_TODO.md](DOING_UNDO_TODO.md)）。  
> **本引擎定位：** 桌面 **渲染中台**（Win D3D12+Vulkan），**不是** 全能游戏引擎。  
> 相关：[POSITIONING.md](POSITIONING.md)、[KNOWN_GAPS.md](KNOWN_GAPS.md)、[PLAN.md](PLAN.md) §1.9。

## 1. 两把尺子

| 尺子 | 比什么 | 不比什么 |
|---|---|---|
| **渲染内核** | 光栅主路径、光影、后处理、GI/RT 深度、GPU Driven、双后端 | 编辑器、脚本、导出、生态、玩法 |
| **完整游戏引擎** | 上者 + 编辑器 + 脚本一等公民 + 音频/多人/平台导出 + 资产生态 | — |

百分比 =「相对该参照引擎产品能力面的可感知重合」，**本口径封板 100% ≠ 对标主流 100%**。

## 2. 综合完成度

| 参照 | 当渲染内核 | 当完整游戏引擎 | 读法 |
|---|---:|---:|---|
| **Godot 4** | **约 60–70%** | **约 35–45%** | 渲染面最接近；输在编辑器/2D/脚本/导出 |
| **Unity（URP/HDRP 综合）** | **约 45–55%** | **约 20–30%** | 内核差一截；整引擎再扣 Editor/C#/生态/平台 |
| **UE5** | **约 25–35%** | **约 10–20%** | 内核被 Lumen/Nanite/TSR 拉开；整引擎再被编辑器/蓝图/复制碾压 |

**一句话：**

- Godot：内核六七成，整引擎三四成。  
- Unity：内核大约一半，整引擎两三成。  
- UE5：内核大约三成，整引擎一两成。

## 3. 分域粗表（vs 各参照的「渲染向」）

| 功能域 | vs Godot | vs Unity | vs UE5 | 本引擎要点 |
|---|---:|---:|---:|---|
| 基础 3D 光栅 | 70–80% | 60–70% | 50–60% | Forward+ / CSM / IBL |
| 后处理 | 65–75% | 55–65% | 40–50% | SSAO/TAA/SSR/Bloom… |
| 动态 GI | 40–55% | 25–35% | 15–25% | DDGI-lite；非 Lumen/真 DDGI |
| 光追 | 30–45% | 25–35% | 15–25% | DXR 示范 + 软影 half-res |
| GPU Driven / 几何 | 50–60% | 40–50% | 20–30% | Cull/Indirect；**无 Nanite** |
| 超分 | 20–30% | 5–15% | 5–10% | bilinear；**DLSS/FSR2 冻结** |
| 2D | 45–55% | 40–50% | 35–45% | 有混合；弱于 Godot 2D 产品化 |
| 物理 | 55–65% | 45–55% | 35–45% | Jolt；无载具/服装管线 |
| 音频 | 20–30% | 15–25% | 10–20% | 播控 only；不做 DSP |
| 编辑器/工具 | 15–25% | 10–20% | 5–15% | CLI + 外挂 editor |
| 脚本/玩法（引擎内） | ~0% | ~0% | ~0% | 外挂 `game_kit` |
| 网络/复制 | 15–25% | 10–20% | 5–15% | HTTP/WS；无状态同步 |
| 平台覆盖 | 30–40% | 25–35% | 20–30% | Win+Linux；无 mac/移动 |
| 双后端对齐纪律 | 80–100% | 70–90% | 60–80% | VULKAN_PARITY + Feature/SKIP |

## 4. 明确不做（拉低「整引擎」分数的主因）

- Nanite / 真 NVIDIA DDGI / Lumen 级 GI  
- DLSS / FSR2 / Frame Generation（冻结）  
- macOS / 移动 / Metal  
- 引擎内脚本 VM、完整可视化编辑器、状态同步/复制  
- 音频 DSP / 商业资产生态  

详见 [POSITIONING.md](POSITIONING.md) §2–3、[DOING_UNDO_TODO.md](DOING_UNDO_TODO.md) 冻结/外置表。

## 5. 工作区补一层之后

| 层 | 对「完整游戏引擎」分数的影响 |
|---|---|
| `game_kit` GK0–GK5 已接线 | 略抬脚本/关卡流，仍远低于 Unity/Godot 一等公民 |
| `editor` ED 已收口 | 略抬摆关，仍远低于三大编辑器 |
| `genre_kits` / 新品类 | 仍弱项（暂停） |

口径：[PLAN.md](PLAN.md) **§1.9**。

## 6. 修订

若水位大变（解冻超分、真 GI、开放世界几何等），更新本页日期与表格，并在 [DOING_UNDO_TODO.md](DOING_UNDO_TODO.md) Done 留一行。
