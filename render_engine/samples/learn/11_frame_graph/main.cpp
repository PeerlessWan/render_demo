...
int main(int argc, char** argv) {
  ...
  desc.window.title = "Learn 11 — Frame Graph";
  ...
  const auto status = a.Run([&](engine::Application& app_ref) {
    ...
    engine::LogInfo("FrameGraph passes this frame: " +
                    std::to_string(render.frame_graph().order().size()) +
                    " draws=" + std::to_string(render.last_draw_count()));
  });
  return status ? 0 : 1;
}
