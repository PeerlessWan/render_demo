...
engine::render::RenderSystemDesc LitDesc() {
  ...
  r.quality.multithread_submit = true;
  return r;
}
...
int main(int argc, char** argv) {
  ...
  desc.window.title = "Learn 27 — GPU Submit MT";
  ...
  cfg.worker_count = 2;
  if (auto st = a.device().SetSubmitConfig(cfg); !st) {
    engine::LogError(st.message());
    return 1;
  }
  engine::LogInfo("SetSubmitConfig multithread=true workers=2");
