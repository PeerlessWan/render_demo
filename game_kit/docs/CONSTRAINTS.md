...
1. 只依赖 `render_engine` **公开头**与文档约定的 Host API **v0.1**（`engine::kHostApiVersion`）。  
2. 禁止依赖 `render_engine/engine/backends/**`、引擎内部匿名命名空间符号。  
3. 三方：Lua 仅出现在 `game_kit/script`；Recast/Detour 经 FetchContent 链入导航，不直含 Jolt。  
...
1. 不实现渲染特性补丁「绕过引擎质量档」。  
2. 导航：本层 Recast 烘焙 + Steer 回退；不提供离线编辑器。联机：进程内 `ReplicationSession`，不是独立游戏服。  
...
1. 依赖 Host API **0.1**（Scene TRS、Input、Physics Raycast/MoveCharacter/Overlap/Contacts、Retained UI、Audio Play、Asset RequestLoad）。  
2. 破坏性绑定变更走 game_kit 自身 semver，不偷偷依赖引擎私有布局。  
3. Prefab/场景格式遵循 [PREFAB_SCHEMA.md](../../render_engine/docs/PREFAB_SCHEMA.md) **v3 Frozen**。  
