...
| GK-G08 | 绑定生成器 | **已写** `api.json` → `lua_api_reg.inc` + `lua_api.gen.cpp`（变参/协程仍 `hand`） | GK5 |
| GK-G09 | 无品类 kit | 多类型复用时按 [LAYERS](../../docs/LAYERS.md) 建 `genre_kits/*`；**本波暂停**，不阻塞本层 | ADR 0028 |
| GK-G10 | 骨骼动画绑定 | **已写** `AnimPlayer` 包引擎 `AnimationStateMachine`；Notify / 根运动 | SCRIPTING |
| GK-G11 | 脚本调试器 / Luau | **DAP 子集** + TCP `Content-Length` + [`tools/vscode-game-kit-lua`](../tools/vscode-game-kit-lua/) 骨架；不换 Luau、不发市场 | SCRIPTING |
